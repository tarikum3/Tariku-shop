...
const OrderScreen = ({ match, history }) => {
  ...
  const makePayment = async () => { 
    ...
    const body = { product:{name:"Cloth",price:order.totalPrice,quantity:order.orderItems.length} }; 
    const headers = { 
   ...
   // console.log("clicked2"); 
    const response = await fetch( 
      "http://localhost:3000/api/users/create-checkout-session", 
      { 
        method: "POST", 
        headers: headers, 
